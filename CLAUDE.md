...
- `css/theme.css` is the active design system entry point. It `@import`s `css/flowbite-theme.css` (full Flowbite Design System token set: radius scale, neutrals, brand, status, dark-mode overrides) and then adds MarketData-specific tokens that Flowbite does not provide: brand hexes, the Quicksand display face, Docusaurus admonition colors, custom shadows and the focus ring. It overrides nothing. Consumed by projects that run their own Tailwind build.
- `css/flowbite-theme.css` is Flowbite v4's **modern** theme — which upstream ships as `src/themes/default.css`; there is no `modern.css` — with our changes applied **in place**, not overridden downstream. We diverge in exactly three ways: `danger` is Tailwind's red rather than Flowbite's rose, the font stack is system fonts rather than Inter, and the theme block is `@theme static` rather than `@theme`. Brand tokens are Flowbite's, unmodified.
- Both theme blocks (`flowbite-theme.css` and `theme.css`) are `@theme static`. A plain `@theme` is tree-shaken to the tokens a generated utility uses, while the `.dark` blocks are plain CSS rules that always ship whole — which left 49 tokens declared for dark and undefined for light (issue #50). Keep `static` on both. `tests/unit/theme-token-parity-css.test.js` guards it.
- `css/flowbite-theme.upstream.css` is a byte-identical copy of that upstream file (currently flowbite@4.0.2), kept so the divergence is one command away: `diff -w css/flowbite-theme.upstream.css css/flowbite-theme.css`. Nothing imports it and nothing builds from it. It is in `.prettierignore` so it stays byte-identical, and `@source not` in `css/components.input.css` keeps source detection from emitting the `rose-*` palette it names. Do not edit it — replace it wholesale when refreshing Flowbite, then reapply our three changes to the derived file.
...
- Every e2e spec calls `watchConsole(test)` (`tests/e2e/helpers/console.js`): console errors and warnings, plus `pageerror`, fail the spec. Silence a single message with `allow: ['<substring>']` — never by level. A dead allowlist entry fails the run, and a green run prints what it suppressed. See README.md "What the tests hear".
- `playwright.config.js` launches Chromium with `--force-renderer-accessibility`. Chrome skips the accessibility tree in a default headless launch, so accessibility warnings are never emitted at all and the console watch goes silently blind. `tests/e2e/console-watch.spec.js` fails if that flag is dropped. Do not remove either.
