...
- `css/flowbite-theme.css` is Flowbite v4's **modern** theme — which upstream ships as `src/themes/default.css`; there is no `modern.css` — with our changes applied **in place**, not overridden downstream. We diverge in exactly two ways: `danger` is Tailwind's red rather than Flowbite's rose, and the font stack is system fonts rather than Inter. Brand tokens are Flowbite's, unmodified.
- `css/flowbite-theme.upstream.css` is a byte-identical copy of that upstream file (currently flowbite@4.0.2), kept so the divergence is one command away: `diff -w css/flowbite-theme.upstream.css css/flowbite-theme.css`. Nothing imports it and nothing builds from it. It is in `.prettierignore` so it stays byte-identical, and `@source not` in `css/components.input.css` keeps source detection from emitting the `rose-*` palette it names. Do not edit it — replace it wholesale when refreshing Flowbite, then reapply our two changes to the derived file.
