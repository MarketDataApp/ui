...
- **Search Input**: `.search-input-group` (on the `<form>`), `.search-input`, and three buttons — `.search-input-button` (label), `.search-input-button-icon` (glyph + label), `.search-input-button-compact` (glyph only). All three take the same markup, `<button class="…">Search</button>`; the class picks the presentation. The magnifier is a `mask-image` on `::before`, the way `.copy-icon` carries its clipboard, so there is no SVG to paste and it follows `currentColor`. The compact variant collapses the label with `font-size: 0` rather than removing it, so the text is still the accessible name — no `aria-label`, and no `aria-hidden` on a glyph, because a pseudo-element is already decorative to assistive tech. A text field with a submit button joined to its trailing edge. `.search-input` composes `.form-input` for its behaviour — focus ring, autofill repaint, disabled and read-only states — and mirrors Flowbite's own search input for its surface colours, using `bg-neutral-secondary-medium` and `border-default-medium` rather than `form-input`'s raw `gray-*` pairs. Both tokens are theme-aware, so there is no `dark:` variant to keep in step. Both halves render 42px. Note that this boundary measures 1.18:1 light and 1.42:1 dark against its own fill, under WCAG 1.4.11's 3:1 — a deliberate choice to match Flowbite, and in dark the fill equals `card-surface`, so on a raised panel the field has no edge. The field rings on focus in `--color-brand` like every other field; the button takes a 4px `brand-medium` halo instead, because its own fill is `brand-strong` and a `brand` ring against it measures 1.29:1 and disappears. Each half lifts itself with `z-index` so its ring survives the join. No JS.
