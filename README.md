...
- **Search Input**: `.search-input-group` (on the `<form>`), `.search-input`, `.search-input-button`, `.search-input-button-compact` (icon-only). A text field with a submit button joined to its trailing edge. `.search-input` composes `.form-input` and changes only the shared edge, the boundary contrast, and the webkit search decorations, so a search field and a plain field cannot drift apart. Both halves render 42px. The field rings on focus in `--color-brand` like every other field; the button takes a 4px `brand-medium` halo instead, because its own fill is `brand-strong` and a `brand` ring against it measures 1.29:1 and disappears. Each half lifts itself with `z-index` so its ring survives the join. No JS.
